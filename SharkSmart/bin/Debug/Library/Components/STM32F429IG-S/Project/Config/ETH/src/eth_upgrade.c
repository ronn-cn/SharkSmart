...
struct http_message_app_info_t message_app_info;
struct http_message_resources_info_t message_resources_info;
struct http_url_info_t URL_LIST[URL_CHOOSE_MAX];
uint32_t loacl_version = 0;
		...
		while(1)
		{
			...
			if(!(fno.fattrib & AM_DIR))
            {
                ...
                for(uint8_t i = 0; i < message->files_cnt; i++)
                {
                    ...
                    {
                        for(uint8_t j = i; j < message->files_cnt - 1; j++)
                        {
...
uint8_t find_version(void)
{
    ...
    // }
    Config_Read(FIRMWARE_VERSION_ADDR,&loacl_version,FIRMWARE_VERSION_SIZE);
...
uint8_t EV_Add_Updata_Info_Manage(char *Path,char *f_Name,uint16_t Port)
{
    ...
    if(strstr(Path,"www") == NULL)
    {
        LOG(LEVEL_INFO,"file path err!!!\n");
        return 2;
    }
    
    sscanf(Path,"%[^/]%s",f_host_name,f_url1_1);
    LOG(LEVEL_INFO,"%s\n%s\n",f_host_name,f_url1_1);
    for(url_cnt=0;url_cnt<URL_CHOOSE_MAX;url_cnt++)
    {
...
void lwip_upgarde_thread(void *arg)
{
    ...
    while(1)
    {
        ...
        if((link_host = gethostbyname(URL_LIST[url_choose].host_name)) == NULL)
        {
