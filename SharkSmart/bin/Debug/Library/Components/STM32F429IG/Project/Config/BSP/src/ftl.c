...
uint8_t FTL_Init(void)
{
	...
	if(nand_dev.lut)myfree(SRAMEX,nand_dev.lut);
    ...
    if(temp) 
    {   
        LOG(LEVEL_DEBUG,"app FTL_CreateLUT err!\r\n");
		LOG(LEVEL_DEBUG,"app voltag instability\r\n");
        // temp=FTL_Format();     //格式化NAND
        // if(temp)
        // {
        //     LOG(LEVEL_DEBUG,"format failed!\r\n");
        //     return 2;
        // }
    }else 	//创建LUT表成功
	{
//		LOG(LEVEL_DEBUG,"total block num:%d\r\n",nand_dev.block_totalnum);
//		LOG(LEVEL_DEBUG,"good block num:%d\r\n",nand_dev.good_blocknum);
//		LOG(LEVEL_DEBUG,"valid block num:%d\r\n",nand_dev.valid_blocknum);
    }
	return 0;
} 
...
uint8_t FTL_CreateLUT(uint8_t mode)
{
    ...
    for(i=0;i<nand_dev.block_totalnum;i++)
    {
        ...
        {        
            //LOG(LEVEL_DEBUG,"bad block index:%d\r\n",i);
        } 
} 
    //LUT表建立完成以后检查有效块个数
    for(i=0;i<nand_dev.block_totalnum;i++)
...
uint8_t FTL_BlockCompare(uint32_t blockx,uint32_t cmpval)
{
	...
	for(i=0;i<3;i++)//允许3次机会
	{
		...
		if(res) 
		{
			LOG(LEVEL_DEBUG,"error erase block:%d\r\n",i);
		}
		else
		{ 
	...
	LOG(LEVEL_DEBUG,"bad block checked:%d\r\n",blockx);
	return 1;
}
...
uint8_t FTL_Format(void)
{
    ...
    for(i=0;i<nand_dev.block_totalnum;i++)	
    {
        ...
        { 
			...
			if(temp)						//擦除失败,认为坏块
			{
				LOG(LEVEL_DEBUG,"Bad block:%d\r\n",i);
				FTL_BadBlockMark(i);		//标记是坏块
			}else nand_dev.good_blocknum++;	//好块数量加一 
		}
	} 
#endif
    LOG(LEVEL_DEBUG,"good_blocknum:%d\r\n",nand_dev.good_blocknum); 
