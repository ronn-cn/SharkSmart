...
{ 
	uint8_t data;
	// WM_IIC_Start(); 
	// WM_IIC_Send_Byte((WM8978_ADDR<<1)|0);//发送器件地址+写命令	 
	// if(WM_IIC_Wait_Ack())return 1;	//等待应答(成功?/失败?) 
    // WM_IIC_Send_Byte((reg<<1)|((val>>8)&0X01));//写寄存器地址+数据的最高位
	// if(WM_IIC_Wait_Ack())return 2;	//等待应答(成功?/失败?) 
	// WM_IIC_Send_Byte(val&0XFF);	//发送数据
	// if(WM_IIC_Wait_Ack())return 3;	//等待应答(成功?/失败?) 
    // WM_IIC_Stop();
	data = val&0XFF;
	if(WM_IIC_Write(WM8978_ADDR<<1, (reg<<1)|((val>>8)&0X01), &data, 1) != 0)
	return 1;
