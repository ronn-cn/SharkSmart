...
//IO方向设置
#define CT_SDA_IN()  {GPIOI->MODER&=~(3<<(8*2));GPIOI->MODER|=0<<8*2;}	//PI8输入模式
#define CT_SDA_OUT() {GPIOI->MODER&=~(3<<(8*2));GPIOI->MODER|=1<<8*2;} 	//PI8输出模式
//IO操作函数	 
#define CT_IIC_SCL  PCout(13)  //SCL
#define CT_IIC_SDA  PIout(8)  //SDA	 
#define CT_READ_SDA PIin(8)   //输入SDA 
